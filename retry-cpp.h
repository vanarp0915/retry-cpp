...
auto retry(Func func, retry_parameter& retry_value, Args... args) {
    int attempt = 0;
    int wait_time = retry_value.wait_before_retry;
    ...
    while (true) {
        try {
            ...
            if (retry_value.wait_before_retry > 0) {
                if (retry_value.expoential_delay) {
                    wait_time = retry_value.wait_before_retry * retry_value.expoential_delay_factor;
                } 
